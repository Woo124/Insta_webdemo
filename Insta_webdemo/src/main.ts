...
class ResponsiveHandler {
  ...
  private setupResponsive() {
    const checkScreenSize = () => {
      ...
      if (isMobile) {
        navTexts.forEach(text => (text as HTMLElement).style.display = 'none');
        if (logo) (logo as HTMLElement).style.display = 'none';
      } else {
        navTexts.forEach(text => (text as HTMLElement).style.display = 'block');
      ...
      if (rightSidebar) {
        (rightSidebar as HTMLElement).style.display = isTablet ? 'none' : 'block';
      }
    };
